...
<article class="cl pd-20">
    <form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
        <input type="hidden" value="{{$info->role_right_id}}" name="role_right_id">
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                添加权限：
                @foreach($all as $v)
                    <input type="checkbox" {{in_array($v->right_id, $info->right_id)?'checked':''}} class="right_id" name="right_id" value="{{$v->right_id}}" >{{$v->right_name}}
                @endforeach
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        $("#sub").click(function () {
            ...
            var role_right_id={{$info->role_right_id}};
            var url = "/admin/roleright/edit2";
            $.ajax({
                type: "post",
                data: {role_right_id:role_right_id,role_id:role_id,right_id:right_id},
                url: url,
                dataType: "json",
                success: function (msg) {
                    if(msg.code == 200){
